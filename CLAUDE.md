...
- Seed Firestore with the canonical sample data (idempotent, safe to re-run):
  ...
  ```
  This script writes as an unauthenticated client. **`firestore.rules` is now deployed with Week 8 per-role rules** (see Authentication below) and rejects unauthenticated writes, so this script **will fail as-is** — it needs to be updated to sign in first (e.g. `signInWithEmailAndPassword` with a test account) before it can seed/reseed data. Note this alone won't be enough for the `users` collection: the Week 8 rule requires `request.auth.uid == userId` on create, but the seed script writes fixed doc IDs (`u001`, `u002`, `u003`) that don't match any real Firebase Auth uid — seeding `users` docs now needs an authenticated admin path (e.g. the Admin SDK, which bypasses rules) rather than the client SDK the rest of the script uses.
...
- `firestore.rules` + `firebase.json` + `.firebaserc` — now the **Week 8 per-collection/per-role rules**, matching `ACL.md`'s table exactly (`users` readable/creatable only by the owning uid, role forced to `employee` on create; `leaveTypes` readable by anyone signed in, writable only by `hr`; `leaveRequests` readable by its own `requesterId` or by `manager`/`hr`, creatable only as your own doc starting at `รอพิจารณา`, updatable only by `manager`/`hr` and only the `status` field — pending → approved/rejected only — or the `aiSuggestion` field, deletable only by the owner while still `รอพิจารณา`; `approvals`/`aiLog` subcollections mirror the parent request's read/write visibility). **Deployed** (`firebase deploy --only firestore:rules` run against `leaveeasy-jakrapan`). Verified manually: signed in as an `employee` test account and opened another user's `leaveRequests/{id}` directly by URL — Firestore now returns `permission-denied` before any client-side role check runs (see the `err.code === "permission-denied"` branch in `js/leave-request-detail.js`).
- Because rules now scope `leaveRequests` reads by owner, `js/leave-requests.js` can no longer do a plain unfiltered `getDocs(collection(db, "leaveRequests"))` for an `employee` — Firestore rejects a collection query that isn't provably restricted to docs the rule allows. It now runs `query(collection(db, "leaveRequests"), where("requesterId", "==", uid))` for employees and the unfiltered query only for `manager`/`hr`. Keep this in mind for any other page that queries `leaveRequests` as a plain employee later — it needs the same `where` clause, not client-side filtering after the fact.

### Role-based UI restrictions (UI + Firestore Security Rules — see `ACL.md`)

`ACL.md` is the source of truth for what each of the 3 roles (`employee`/`manager`/`hr`) can and can't do. As of now:

- `js/leave-requests.js` filters the table to `requesterId === ผู้ใช้.uid` when `role === "employee"`; `manager`/`hr` see everything. (Backed by a real `where` query for employees now — see Authentication above.)
...
**As of Week 8, the `leaveRequests`/`users`/`leaveTypes` read/write restrictions above are now also enforced server-side by `firestore.rules`** (see Authentication above) — someone opening devtools and calling the Firestore SDK directly to read/write another user's request, change a status field beyond what's allowed, or self-assign a non-`employee` role now gets `permission-denied` from Firestore itself, not just a hidden UI element. The UI checks above still matter for UX (friendly messages, hidden buttons) — the rules are the actual security boundary now. Two things Week 8 rules deliberately do **not** re-enforce server-side (still client-only, same as before): the "reject requires ≥1 existing `approvals` entry" business rule in `เปลี่ยนสถานะ()` (would need a subcollection count, which Firestore rules can't do cheaply), and new comments still aren't written to Firestore at all yet (see Migration state below) so there's nothing to secure there yet.
