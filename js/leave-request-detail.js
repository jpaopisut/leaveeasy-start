...
// หัวหน้า/ฝ่ายบุคคล, ปุ่มลบอิงความเป็นเจ้าของ (requesterId) ไม่อิง role — สัปดาห์ที่ 8
// Firestore Security Rules บังคับเรื่องนี้จริงแล้วเช่นกัน (getDoc ด้านล่างจะโดน
// permission-denied ก่อนถึงเช็ค role นี้ด้วยซ้ำ ถ้าเปิดใบของคนอื่น) เช็คนี้จึงเหลือไว้แค่โชว์
// ข้อความ/ซ่อนปุ่มให้ UX ดี ไม่ใช่ชั้นความปลอดภัยเดียวอีกต่อไป
// ─────────────────────────────────────────────────────────────
...
(async function () {
  ...
  try {
    ...
    // สัปดาห์ที่ 8: พนักงานเปิดใบของคนอื่นตรงๆ ด้วย URL จะโดน Firestore Security Rules
    // ปฏิเสธตั้งแต่ตอน getDoc เลย (permission-denied) ก่อนที่โค้ดเช็ค role ด้านล่างจะได้ทำงานด้วยซ้ำ
    // แสดงข้อความเดียวกับตอนเช็ค role ฝั่ง UI เพื่อไม่ให้ผู้ใช้เห็น error ดิบของ Firestore
